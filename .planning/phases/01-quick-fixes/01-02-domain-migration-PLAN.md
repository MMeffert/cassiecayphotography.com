...
<task type="auto">
  ...
  Steps:
  1. First, disable the old distribution (required before deletion):
     ```bash
     export AWS_PROFILE=personal
     aws cloudfront get-distribution-config --id ED1MCQW8EZ1X1 > /tmp/cf-config.json
     ```

  2. Extract the ETag and update config to disable:
     ```bash
  ...
  3. Wait for distribution to fully disable (Status: Deployed):
     ```bash
     aws cloudfront wait distribution-deployed --id ED1MCQW8EZ1X1
     ```

  4. Delete the disabled distribution:
     ```bash
  ...
  <verify>
  1. `aws cloudfront get-distribution --id ED1MCQW8EZ1X1` returns error (distribution not found)
